...
void main() async {
  ...
  initLogging();
  log.info('[Boot] Flutter binding initialized (TV)');
  ...
  // Android TV: landscape-only, no immersive mode (system bars visible for leanback)
  if (Platform.isAndroid) {
    await SystemChrome.setPreferredOrientations([
      DeviceOrientation.landscapeLeft,
      DeviceOrientation.landscapeRight,
    ]);
    // Keep system UI visible on TV — no immersive mode
    await SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge);
  }

  // Configure InAppWebView for stream extraction
  if (Platform.isAndroid) {
    try {
...
class _SplashScreenState extends ConsumerState<SplashScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initEngine() async {
    log.info('[Boot] Starting TV engine initialization...');
