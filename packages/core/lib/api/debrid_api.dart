...
class DebridApi {
  ...
  final String _rdClientId = ApiKeys.rdClientId;
  ...
  Future<bool> _exchangeRDToken(String deviceCode, String clientId, String clientSecret) async {
    const url = 'https://api.real-debrid.com/oauth/v2/token';
  ...
  Future<String?> getRDAccessToken() async {
    ...
    if (DateTime.now().isAfter(expiry.subtract(const Duration(minutes: 5)))) {
      ...
      final refreshed = await _refreshRDToken();
      if (refreshed != null) return refreshed;
      // Refresh failed — return current token as last resort
      return token;
    }
    return token;
  }
  ...
  Future<String?> _refreshRDToken() async {
    final refreshToken = await _storage.read(key: 'rd_refresh_token');
    final clientId = await _storage.read(key: 'rd_client_id');
    final clientSecret = await _storage.read(key: 'rd_client_secret');
    ...
    try {
      final response = await http.post(
        Uri.parse('https://api.real-debrid.com/oauth/v2/token'),
        body: {
          'client_id': clientId,
          'client_secret': clientSecret,
          'grant_type': 'http://oauth.net/grant_type/device/1.0',
