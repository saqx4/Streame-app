...
void main() async {
  ...
  if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
    ...
    WindowOptions windowOptions = WindowOptions(
      ...
      center: winX == null, // center only if no saved position
...
class _StreameAppWrapperState extends State<_StreameAppWrapper> with WidgetsBindingObserver, WindowListener {
  ...
  void onWindowClose() async {
    if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
      ...
      if (isPreventClose) {
        // Run cleanup with a 2-second timeout. If it finishes, great.
        // If not, force-exit anyway — the OS will reclaim resources.
        try {
          await Future.any([
  ...
  /// Persist window position and size so the app reopens where the user left it.
...
class _SplashScreenState extends ConsumerState<SplashScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initEngine() async {
    ...
    log.info('[Boot] Step 1: Checking network connectivity...');
    final connectivityResult = await Connectivity().checkConnectivity();
    final isOffline = connectivityResult.contains(ConnectivityResult.none);
    log.info('[Boot] Network status: ${isOffline ? "OFFLINE" : "ONLINE"}');

    if (isOffline) {
      log.info('[Boot] Device is offline, initializing local services only');
      log.info('[Boot] ✓ Local services initialized');
      if (mounted) {
        log.info('[Boot] Navigating to MainScreen (offline mode)');
    ...
    log.info('[Boot] Step 2: Starting essential services...');
    ...
    log.info('[Boot]   TorrentStream: ✓ started');
    log.info('[Boot]   LocalServer: ✓ started');

    _updateProgress(1.0, 'Ready!');
    
    if (mounted) {
      log.info('[Boot] Step 6: Navigating to MainScreen...');
      Navigator.of(context).pushReplacement(
        PageRouteBuilder(
      ...
      log.info('═══════════════════════════════════════════════════════════');
      log.info('[Boot] ✓✓✓ ENGINE INITIALIZATION COMPLETE ✓✓✓');
      log.info('═══════════════════════════════════════════════════════════');
    }
  }
