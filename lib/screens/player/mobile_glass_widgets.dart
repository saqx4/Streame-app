...
class CleanContainer extends StatelessWidget {
  ...
  const CleanContainer({super.key, 
    required this.child,
    this.radius = 12,
    this.padding,
    this.tint,
...
class CleanBlurContainer extends StatelessWidget {
  ...
  const CleanBlurContainer({super.key, 
    required this.child,
    this.radius = 12,
    this.padding,
  });
...
class GlassPillButton extends StatefulWidget {
  ...
  const GlassPillButton({super.key, 
    required this.text,
    required this.onTap,
    this.accent,
  });
...
class GlassPlayPauseState extends State<GlassPlayPause> {
  ...
  Widget build(BuildContext context) {
    if (widget.isBuffering) {
      return const CleanBlurContainer(                     // ← blur OK, only 1 on screen
        radius: 32,
        child: SizedBox(
          width: 64,
          height: 64,
